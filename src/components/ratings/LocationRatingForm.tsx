...
const CRITERIA = [
  ...
  { key: 'infrastructure', label: 'Sistema/Infraestrutura' },
  { key: 'organization', label: 'Organização' },
  { key: 'safety', label: 'Segurança' },
];

interface LocationRatingFormProps {
  locationName: string;
  scaleTitle: string;
  onSubmit: (data: {
    overallScore: number;
...
export function LocationRatingForm({ locationName, scaleTitle, onSubmit, onCancel }: LocationRatingFormProps) {
  const [scores, setScores] = useState<Record<string, number>>({ cleanliness: 0, infrastructure: 0, organization: 0, safety: 0 });
  ...
  const handleSubmit = () => {
    onSubmit({ overallScore, scores, comment });
  };

  return (
    <Card className="glass-card">
      <CardHeader>
        <CardTitle className="text-lg">Avaliar Local</CardTitle>
        <p className="text-sm text-muted-foreground">{locationName} · {scaleTitle}</p>
      </CardHeader>
